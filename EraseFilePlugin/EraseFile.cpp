...
#include <time.h>
#include <assert.h>
CEraseFile::CEraseFile(QObject* parent)
{
  MemmoryAlloc();
  GetNativeEntryPoints();
  m_realcompress = false;
  m_zap = false;
  bool  success = connect(this, SIGNAL(UpdataProgressValue(int)),parent, SLOT(OnProgressValue(int))); 
  assert(success == true);
}

CEraseFile::~CEraseFile(void)
{
  for(int j = 0; j < 3; j++ ) 
  {
...
bool CEraseFile::GetNativeEntryPoints()
{
  if( GetVersion() >= 0x80000000) 
      return false;
  
  if(!(m_NtFsControlFile = (PNtFsControlFile) GetProcAddress( GetModuleHandle(L"ntdll.dll"),
    "NtFsControlFile" ) )) 
  {
     return false;
  }
  if( !(m_RtlNtStatusToDosError = (PRtlNtStatusToDosError) GetProcAddress( GetModuleHandle(L"ntdll.dll"),
    "RtlNtStatusToDosError" )) ) 
  {
    return false;
  }

// m_PGetDiskFreeSpaceEx = (PGetDiskFreeSpaceEx) GetProcAddress(GetModuleHandle(L"ntdll.dll"),"GetDiskFreeSpaceExA");
  return true;
}

void CEraseFile::GetFiels(QString odir, QStringList& absolutePaths)
{
  QDir dir(odir);
...
bool CEraseFile::StartEraseFile(QStringList EraseList)
{
  ...
  {  
     ...
     if (FileAttr & FILE_ATTRIBUTE_COMPRESSED ||
         FileAttr & FILE_ATTRIBUTE_SPARSE_FILE ||
         FileAttr & FILE_ATTRIBUTE_ENCRYPTED)
     {
...
QString CEraseFile::PrintWin32Error( DWORD ErrorCode )
{
  LPVOID lpMsgBuf;
...
void CEraseFile::CleanFreeSpace()
{
  QString DrivePath = "D:\\";
  ...
  if( !GetDiskFreeSpace(DrivePath.toStdWString().c_str(), &sectorsPerCluster, &bytesPerSector,
  ...
  hTempFileOne = CreateFile(tempFileNameOne.toStdWString().c_str(), GENERIC_WRITE, 
  ...
  while (cleanSize > sectorsPerCluster * bytesPerSector)
  {
      if (SafeWrite(hTempFileOne,cleanSize))
      {
        ...
        emit UpdataProgressValue(percent);
      }
      else
      {
  ...
  hTenpFileSecond = CreateFile(tempFileNameSecond.toStdWString().c_str(), GENERIC_WRITE, 
    0, NULL, CREATE_NEW, 
  ...
  while(true) 
  {
    ...
    HANDLE hTempFile = CreateFile(tempFileNameMFT.toStdWString().c_str(), GENERIC_WRITE, 
      0, NULL, CREATE_NEW, 
      FILE_FLAG_SEQUENTIAL_SCAN|FILE_FLAG_DELETE_ON_CLOSE|
...
bool CEraseFile::SafeRename(QString FileName, QString& Lastname)
{
  QString NewFilename;
...
FileHandle 文件句柄
length 写入文件的长度
SafeWrite 向文件指定位置写入length数据3次。
第一次0x00 第二次写入0xff 第三次写入随机数据
*/
bool CEraseFile::SafeWrite(HANDLE FileHandle, qint64 length)
{
...
bool CEraseFile::SafeDeleteCompressed(QString FileName)
{
  HANDLE			hFile;
  ...
  */
  ...
  hFile = CreateFile( FileName.toStdWString().c_str() , GENERIC_READ, 
...
bool CEraseFile::SafeWriteCompressedFile(HANDLE VolumeHandle, 
                             HANDLE hFile,
                             DWORD clusterSize, 
                             bool* realcompress, 
                             bool* zap)
{
  ...
  *zap = false;
  ...
  while(!(status = m_NtFsControlFile( hFile, NULL, NULL, 0, &ioStatus,
...
bool CEraseFile::SafeDelete(QString FileName, quint64 length)
{
  HANDLE	hFile = NULL;
